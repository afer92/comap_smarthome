...
class ComapClient(object):
    ...
    def get_housing_thermal_details(self, housing_id):
        ...
        base_url = self._BASEURL + "thermal/"
        url = "{}housings/{}/thermal-details".format(base_url, housing_id)
        return self.get_request(url)

    def get_custom_thermal_temperatures(self, housing_id):
        """Get custom thermal temperatures used for instructions"""
        base_url = self._BASEURL + "thermal/"
        url = "{}housings/{}/custom-temperatures".format(base_url, housing_id)
        return self.get_request(url)
    ...
    def get_zones_z(self, housing=None):
        if housing is None:
            housing = self.housing
        return self.get_request(
            self._BASEURL + "thermal/housings/" + housing + "/zones"
        )
    ...
    def get_connected_object(self, serial):
        url = "{}park/connected-objects/{}"
        url = url.format(self._BASEURL, serial)
        return self.get_request(url)
